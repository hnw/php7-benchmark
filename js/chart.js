...
function drawChart(content) {
	...
	[new Date('2015-03-19'), 0.837, 11.756, 'PHP Australia', '<img src="img/rasmus2015.png" />'],
	[new Date('2015-04-30'), 0.777, 11.081, null, null]
    ];

    if (content == 'bench.php') {
	rows = raw_rows.map(function(arr){return [arr[0],arr[1],arr[3], arr[4]];});
    } else {
	rows = raw_rows.map(function(arr){return [arr[0],arr[2],arr[3], arr[4]];});
    }
    rows = rows.filter(function(arr){return arr[1] != null;});
